...
## Overview
The local provider stores artifacts on disk using a root directory. Each
artifact is stored as a file named after its hash, with metadata stored in a
neighboring `<hash>.json` file.
...
Notes:
- The e2e test writes artifacts under `.e2e-artifacts/` and cleans up on
  success.
