...
The S3 provider stores artifacts in an S3-compatible bucket. Artifact keys are prefixed with `<teamId>/<slug>/` before the hash, and metadata is stored as object metadata (size, duration, and optional tag). Missing scope values fall back to `_`.
