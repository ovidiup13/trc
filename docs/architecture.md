...
TRC is a lightweight, self-hosted Turborepo Remote Cache server. The current implementation focuses on the core API, JWT auth, local storage, config loading, and CLI orchestration.
...
All routes require a JWT bearer token signed with a shared secret (HS256).

- Header: `Authorization: Bearer <token>`
- Missing token -> `401` with `code: "unauthorized"`
- Invalid token -> `401` with `code: "unauthorized"`

Auth is implemented via `jose` and applied globally to the Hono app.
